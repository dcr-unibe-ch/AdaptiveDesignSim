...
the number and potentially time point of interim analyses (*nia*,
*tia*). the distribution (*simfun*) and paremeters in each arm
(*par01*), and the estimation function (*estfun*).
...
We would like to do one interim analysis at half of the patients. Single
simulations under the null and alternative can the be done like that:
...
(*true_pe*, a risk difference in this example), and—for each interim
stage (prefix *iax\_*) and the final stage (prefix *fa\_*)—the
...
- simulated point estimate (suffix *pe*, a risk difference in this
  example),
...
two with the simulated outcomes for control and experimental group.

For example, a Poisson distribution with expected counts of 4 and 3 in
control and experimental group and low counts being better:
